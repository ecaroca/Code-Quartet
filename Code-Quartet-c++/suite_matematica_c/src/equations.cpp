...
// s=1: F1 = x^2 + y^2 - 4,   F2 = std::exp(x) + y - 1
// s=2: F1 = std::sin(x) + y - 1,  F2 = x + std::cos(y) - 1
static void F(int s, double x, double y, double* F1, double* F2){
    switch(s){
        case 1: *F1 = x*x + y*y - 4.0; *F2 = std::exp(x) + y - 1.0; break;
        case 2: *F1 = std::sin(x) + y - 1.0; *F2 = x + std::cos(y) - 1.0; break;
        default: *F1 = *F2 = std::numeric_limits<double>::quiet_NaN();
    }
}
static void J(int s, double x, double y, double* a11,double* a12,double* a21,double* a22){
    switch(s){
        case 1: *a11 = 2*x; *a12 = 2*y; *a21 = std::exp(x); *a22 = 1.0; break;
        case 2: *a11 = std::cos(x); *a12 = 1.0; *a21 = 1.0; *a22 = -std::sin(y); break;
        default: *a11=*a12=*a21=*a22=std::numeric_limits<double>::quiet_NaN();
    }
}
...
int system2_newton(int s, double x0, double y0, double tol, int itmax){
    ...
    for (int it=0; it<itmax; ++it){
        ...
        if (std::fabs(det)<1e-14) return 0;
        // delta = J^{-1} * F
        double dx = ( + a22*F1 - a12*F2)/det;
        double dy = ( - a21*F1 + a11*F2)/det;
        x -= dx; y -= dy;
        if (std::sqrt(dx*dx+dy*dy) < tol){ SX=x; SY=y; return 1; }
    }
    SX=x; SY=y; return 0;
}
