...
static double gauss_elim(Matrix A, int* piv){
    ...
    for (int k=0;k<n;k++){
        // pivot
        int p=k; double maxv=std::fabs(mat_get(A,p,k));
        for (int i=k+1;i<n;i++){
            double v=std::fabs(mat_get(A,i,k));
...
Matrix mat_inv(Matrix A){
    ...
    for (int k=0;k<n;k++){
        int p=k; double maxv=std::fabs(mat_get(AI,p,k));
        for (int i=k+1;i<n;i++){
            double v=std::fabs(mat_get(AI,i,k));
            if (v>maxv){maxv=v;p=i;}
        }
...
Vector mat_solve(Matrix A, Vector b){
    ...
    // Eliminación + pivoteo
    for (int k=0;k<n;k++){
        int p=k; double maxv=std::fabs(mat_get(Aug,p,k));
        for (int i=k+1;i<n;i++){
            double v=std::fabs(mat_get(Aug,i,k));
            if (v>maxv){maxv=v;p=i;}
        }
